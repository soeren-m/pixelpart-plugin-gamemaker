...
GM_EXPORT pixelpart_gm::real GM_API pixelpart_get_image_resource_id(pixelpart_gm::string runtimePtr, pixelpart_gm::real index, pixelpart_gm::string resourceIdBufferPtr, pixelpart_gm::real resourceIdBufferSize) {
	pixelpart_gm::EffectRuntime* effectRuntime = pixelpart_gm::parsePtr<pixelpart_gm::EffectRuntime>(runtimePtr);
	if(!effectRuntime) {
		pixelpart_gm::lastError = pixelpart_gm::invalidEffectRuntimeError;
		return -1;
	...
	for(const auto& entry : effectRuntime->effectAsset.effect().resources().images()) {
		...
		return static_cast<pixelpart_gm::real>(entry.first.size());
	}
